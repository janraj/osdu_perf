...
def run_local_tests(args):
    ...
    # List available locustfiles if requested
    ...
    if not all([args.host, args.partition, args.token]):
        print("❌ Missing required OSDU parameters!")
        print("Required: --host, --partition, --token")
        return
    
    try:
        # Set environment variables for OSDU parameters
        env = os.environ.copy()
        env['OSDU_HOST'] = args.host
        env['OSDU_PARTITION'] = args.partition
        env['ADME_BEARER_TOKEN'] = args.token
        
        if hasattr(args, 'verbose') and args.verbose:
            print("🔧 Environment variables set:")
            print(f"   OSDU_HOST: {args.host}")
            print(f"   OSDU_PARTITION: {args.partition}")
            print(f"   ADME_BEARER_TOKEN: {'*' * 20}...")
        
        # Create a temporary locustfile if none specified
        if hasattr(args, 'locustfile') and args.locustfile and Path(args.locustfile).exists():
            locustfile_path = args.locustfile
            print(f"🎯 Using custom locustfile: {locustfile_path}")
        else:
            ...
            create_locustfile_template(str(locustfile_path))
        ...
        if hasattr(args, 'web_ui') and args.web_ui:
        ...
        # Run locust with environment variables
        print("⚡ Executing locust command...")
        result = subprocess.run(locust_cmd, capture_output=False, text=True, env=env)
        
        if result.returncode == 0:
            print("\n✅ Performance test completed successfully!")
        else:
            print(f"\n❌ Performance test failed with exit code: {result.returncode}")
